...
| ~~R6~~ **DONE slice 1 (batch 156)** | thermo_pw `&INPUT_THERMO` mined from the project's own Fortran ground truth (`thermo_readin.f90` NAMELIST declaration + procedural defaults + verbatim consistency errore lines; `initialize_thermo_work.f90` part-1 dispatch → the 30-value HARD `what` set; 232 keywords with code-group labels + commit/sha256 provenance in `QEThermoPwSchemaData`), with binary-mirrored audit severities (`THERMO_UNKNOWN_KEYWORD`/`THERMO_WHAT_*`/`THERMO_AGGREGATE_RULE` ERROR, flext-remap + inferred-type WARNING) and the auto-GUI-listed `THERMO_PW_DECK_AUDIT` analysis kind routing `thermo_control` files. Per-version thermo_pw windows are NOT claimed (no version tags mined yet — the grammar carries commit provenance, and the class docs say a version layer must announce itself when it lands) | staged master; tags remain |
