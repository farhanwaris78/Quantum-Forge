...
import quantumforge.project.property.ProjectBandFactory;
...
public class QEFXBandViewerController extends QEFXGraphViewerController {
    ...
    protected void reloadData(LineChart<Number, Number> lineChart) {
        ...
        this.showBandGap(bandData1, bandData2, fermi);
    }

    private void showBandGap(BandData bandData1, BandData bandData2, double fermi) {
        double gap = this.calculateBandGap(bandData1, bandData2, fermi);
        if (gap > 0.0) {
            String text = String.format("Band Gap: %.4f eV", gap);
            Platform.runLater(() -> {
                this.stackNode(this.getNote(text), Pos.TOP_RIGHT);
            });
        }
    }

    private double calculateBandGap(BandData bandData1, BandData bandData2, double fermi) {
        if (bandData1 == null) {
            return -1.0;
        }

        double vbm = -Double.MAX_VALUE;
        double cbm = Double.MAX_VALUE;

        int numPoints = bandData1.numPoints();
        for (int i = 0; i < numPoints; i++) {
            double energy = bandData1.getEnergy(i) - fermi;
            if (energy <= 1.0e-6) {
                vbm = Math.max(vbm, energy);
            } else {
                cbm = Math.min(cbm, energy);
            }
        }

        if (bandData2 != null) {
            numPoints = bandData2.numPoints();
            for (int i = 0; i < numPoints; i++) {
        ...
        if (vbm == -Double.MAX_VALUE || cbm == Double.MAX_VALUE) {
            return 0.0;
        }

        return Math.max(0.0, cbm - vbm);
    }
