...
import quantumforge.input.card.QEKPoint;
...
import quantumforge.run.RunningCommand.RunningCommandType;
import quantumforge.run.parser.BandPathParser;
import quantumforge.run.parser.FermiParser;
import quantumforge.run.parser.GeometryParser;
import quantumforge.run.parser.LogParser;
import quantumforge.run.parser.ScfParser;
import quantumforge.run.parser.VoidParser;
...
public enum RunningType {
    ...
    public List<String> getInpNameList(Project project) {
        ...
        case Project.INPUT_MODE_CONVERGENCE:
            for (int i = 0; i < 5; i++) {
                inpList.add(project.getInpFileName("conv_" + i));
            }
            break;
    ...
    public List<PostOperation> getPostList() {
        ...
        case Project.INPUT_MODE_OPTIMIZ:
            postList.add((project) -> {
                this.updateProjectCell(project);
            });
            break;
    ...
    private void updateProjectCell(Project project) {
        ...
        ProjectGeometryList geomList = null;
        if (this == OPTIMIZ) {
            geomList = property.getOptList();
        } else if (this == MD) {
            geomList = property.getMdList();
        }

        if (geomList == null || geomList.numGeometries() < 1) {
            return;
        }

        ProjectGeometry lastGeom = geomList.getGeometry(geomList.numGeometries() - 1);
        if (lastGeom == null) {
            return;
        }
        ...
        double[][] lattice = lastGeom.getCell();
        if (lattice != null) {
            try {
        ...
        int nAtoms = lastGeom.numAtoms();
        Atom[] atoms = cell.listAtoms(true);
        if (atoms != null && atoms.length == nAtoms) {
            for (int i = 0; i < nAtoms; i++) {
                atoms[i].moveTo(lastGeom.getX(i), lastGeom.getY(i), lastGeom.getZ(i));
            }
        }
    }
